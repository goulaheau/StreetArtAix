...
class Evenement
{
    ...
    {
        if (!preg_match("#^[\w\.\!\?\'\#\-\s]{5,}$#", $description)) 
        {
            return false;
        }
    ...
    public function insert()
    {
        ...
        $req = "INSERT INTO evenement(datesql, date, titre, lieu, description, uid) VALUES('".$this->datesql."', '".$this->date."', '".$this->titre."', '".$this->lieu."', '".$this->description."', '".$this->uid."')";
    ...
    {
        if(!is_int($eid)) return array();
        $req = "SELECT uid FROM evenement_utilisateur WHERE eid = $eid ";
        $res = mysqli_query($GLOBALS['db'], $req) or die(mysql_error() . '<br />Erreur dans le fichier ' . __FILE__ . ' à la ligne ' . __LINE__ . ' avec la requete : ' . $req);
        $r = array();
        while($a = mysqli_fetch_assoc($res)){
            $r[] = $a;
